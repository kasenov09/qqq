...
def create_beautiful_buttons(root):
    ...
    def update_button_texts():
        if language == "русский":
            ...
            language_button.config(text="Переключить язык (английский)")
        else:
            button1.config(text="Create")
            button2.config(text="Read")
            button3.config(text="Update")
            button4.config(text="Delete")
            language_button.config(text="Toggle Language (русский)")
    ...
    language_button = ttk.Button(root, text="Переключить язык (английский)", command=on_language_toggle)
