...
def create_beautiful_buttons(root):
    ...
    button1 = ttk.Button(button_frame, style="Beautiful.TButton", command=lambda: on_button_click(1)if )
    ...
    button1.grid(row=6, column=20, padx=50)
    button2.grid(row=2, column=20, padx=50)
    button3.grid(row=2, column=8, padx=50)
    button4.grid(row=6, column=8, padx=50)

    update_button_texts()
    
    language_button = ttk.Button(root, text="LG", command=on_language_toggle)
    
    language_button.grid(row=0, column=0, columnspan=10, pady=0)
